...
const EquipamentCreator = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post("/equipament/", dataToSubmit);
  ...
  const availableDevices = devices.filter(
    (device) => !equipments.some((equip) => equip.device.id === device.id)
  );
      ...
      <div className="container mx-auto my-2">
        ...
        {availableDevices.length === 0 ? (
          <div className="bg-yellow-200 text-yellow-800 p-4 rounded-md">
            <p>
              Você não possui dispositivo disponível no momento. Por favor,
              cadastre novos dispositivos no TRACCAR SERVER.
            </p>
          </div>
        ) : (
